...
from .playstore_proto_pb2 import ResponseWrapper
...
class Playstore(object):
    ...
    def _execute_request(self, path: str, query: dict = None, data: str = None, proxy: str = None) -> object:
        """
        ...
        if data is not None:
            ...
            if proxy:
                response = requests.get(url, headers=headers, params=query, verify=True, proxies={"http": proxy, "https": proxy})
            else:
        ...
        try:
            ...
            raise RuntimeError("Rate limit triggered, will sleep and retry")
        return message
    ...
    def _download_with_progress(
        ...
        response = self._execute_request(path, query, proxy=proxy)
        if response.payload.deliveryResponse.appDeliveryData.downloadUrl:
            ...
            response = self._execute_request(path, data=data, proxy=proxy)
    ...
    def get_store_categories(self, category: str = None, proxy: str = None) -> object:
        """
    ...
    def list_app_by_category(
        self, category: str, subcategory: str = None, num_of_results: int = None, proxy: str = None
    ) -> object:
        """
    ...
    def search(self, query: str, proxy: str = None) -> object:
        """
        ...
        if "payload" not in self.protobuf_to_dict(response):
            try:
                ...
                raise AttributeError("There was an error when searching for '{0}'".format(query))
        else:
            try:
    ...
    def app_details(self, package_name: str, proxy: str = None) -> object:
        """
        ...
        if "payload" not in self.protobuf_to_dict(response):
            try:
                ...
                raise AttributeError("There was an error when requesting details for app '{0}'".format(package_name))
        else:
        ...
        try:
            ...
            list(
                self._download_with_progress(
                    package_name, file_name, download_obb, show_progress_bar, proxy=proxy
                )
            )
        except Exception as e:
            # self.logger.error("Error during the download: {0}".format(e))
            raise e
